...
function MovieDetails() {
  ...
  async function deleteComment(deletedComment) {
    await fetch(
      `http://localhost:3000/movies/${movie.movie_id}/comments/${deletedComment.comment_id}`,
      {
        method: "DELETE",
      }
    );
  ...
  async function createComment(commentAttributes) {
    const response = await fetch(
      `http://localhost:3000/movies/${movie.movie_id}/comments`,
      {
        method: "POST",
        headers: {
  ...
  let rating = <h3 className="inactive">Not yet star rated</h3>;
    ...
    <main>
      <div className="row">
        ...
        <div className="col-sm-6">
          ...
          {movie.genre}
          <br />
          <h3>Rated {movie.rating}.</h3>
          <br />
          <h3>Movie Length {movie.length}.</h3>
          <br />
      ...
      <hr />
      New Comment
      <NewCommentForm movie={movie} onSubmit={createComment} />
    </main>
  );
}
